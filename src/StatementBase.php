...
class StatementBase extends Element {
  ...
  private function validateVoider($errors) {
    ...
    if ($verb_id === 'http://adlnet.gov/expapi/verbs/voided' && $object_type !== 'StatementRef') {
      $errors[] = new Error("`statementobject.objectType` must be `StatementRef` not `$object_type` when voiding");
    }
  ...
  private function validateContextForActivity($errors) {
    ...
    if ($this->getPropValue('context.revision') !== null && !$is_activity) {
      $errors[] = new Error("`context.revision` must only be used if `statementobject.objectType` is `Activity` not `$object_type`");
    }
    if ($this->getPropValue('context.platform') !== null && !$is_activity) {
      $errors[] = new Error("`context.platform` must only be used if `statementobject.objectType` is `Activity` not `$object_type`");
    }
